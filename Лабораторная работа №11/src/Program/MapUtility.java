...
public class MapUtility {
    ...
    public void createMapFromDATFile() {
        try (RandomAccessFile raf = new RandomAccessFile(datFile.getFileNameDat(), "r")) {
            ...
            while ((position = raf.getFilePointer()) < raf.length()) {
                acc = (BankAccount) Buffer.readObject(raf, position);
                if (!acc.isDeleted())
                    this.objectsMap.put(acc.getFieldName(), position);
            }
    ...
    public BankAccount readByKey(Object key) {
        try (RandomAccessFile raf = new RandomAccessFile(datFile.getFileNameDat(), "r")) {
            if (this.objectsMap.containsKey(key)) {
                long position = this.objectsMap.get(key);
                BankAccount acc = (BankAccount) Buffer.readObject(raf, position);
                if (acc.isDeleted())
                    return null;
                else
                    return acc;
    ...
    public void removeByKey(Object key) {
        try (RandomAccessFile raf = new RandomAccessFile(datFile.getFileNameDat(), "rw")) {
            if (this.objectsMap.containsKey(key)) {
                BankAccount acc = readByKey(key);
                if (acc != null) {
                    acc.setDeleted(true);
                    raf.seek(objectsMap.get(key));
                    byte[] temp = Buffer.toByteArray(acc);
    ...
    public void showObjectsAscending() {
        ...
        System.out.println("_______ BankAccounts ascending  _______");
    ...
    public void showObjectsDescending() {
        ...
        System.out.println("_______ BankAccounts descending  _______");
        for (Object entry : entries) {
            System.out.println("Key:  " + entry + " ::  " + readByKey(entry));
        }
        System.out.println("________________________________");
    ...
    public void showObjectBiggerThanKey(Object Key) {
        ...
        System.out.println("_______ BankAccount with key bigger than   " + Key + "  _______");
        int i;
    ...
    public void showObjectSmallerThanKey(Object Key) {
        ...
        System.out.println("_______ BankAccount with key smaller than   " + Key + "  _______");
        int i;
        boolean somethingFound = false;
